...
export function buildReportDoc(report = {}, moduleResults = {}) {
  ...
  return `
      ...
      <body>
        ...
        <div class="section-heading">${verdict ? (moduleRows ? '7' : '6') : (moduleRows ? '6' : '5')}. Citations &amp; References</div>
        <table>
          <tr><th>#</th><th>Source</th></tr>
          <tr><td>[1]</td><td>Client intake &amp; profile — submitted ${esc(date)} by ${or(client.contact || client.company, 'client')} (${or(client.email, 'no email on file')}).</td></tr>
          ${Object.entries(MODULE_TITLES)
            .filter(([key]) => moduleResults[key])
            .map(([key, title], i) => `<tr><td>[${i + 2}]</td><td>${esc(title)} — module output generated ${esc(date)} by The Conscious Orbit intelligence engine.</td></tr>`)
            .join('\n')}
          <tr><td>[${Object.keys(moduleResults).filter((k) => MODULE_TITLES[k]).length + 2}]</td><td>Orbita AI critical analysis (report-based suggestions feeding the AI training loop).</td></tr>
          <tr><td>[${Object.keys(moduleResults).filter((k) => MODULE_TITLES[k]).length + 3}]</td><td>SpyFu competitor intelligence integration (labelled placeholder when API key is not configured).</td></tr>
          <tr><td>[${Object.keys(moduleResults).filter((k) => MODULE_TITLES[k]).length + 4}]</td><td>Google Gemini omni-model routing (gemini-3.6-flash) — content generated at time of export.</td></tr>
        </table>
