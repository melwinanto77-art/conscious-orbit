...
import { MessageSquare, Send, CheckCircle2, Clock, Loader2, Trash2, User, ShieldCheck } from "lucide-react";
        ...
        <div className="space-y-4">
          {visible.map((q) => (
            ...
            >
              ...
              {/* Chat thread — mobilised bubbles */}
              <div className="space-y-2 pt-1">
                {/* Client bubble (left) */}
                <div className="flex items-end gap-2 max-w-full">
                  <div className="shrink-0 h-7 w-7 rounded-full bg-[#4A0A13] text-[#F5D77F] flex items-center justify-center">
                    <User size={13} />
                  </div>
                  <div className="max-w-[85%] sm:max-w-[75%] rounded-2xl rounded-bl-sm bg-[#FAF4E8] border border-[#D4AF37]/40 px-3 py-2">
                    <p className="text-[0.6rem] font-bold uppercase tracking-wider text-[#B8860B]">
                      {q.clientName || (isAdmin ? "Client" : "You")}
                    </p>
                    <p className="text-xs text-[#4A0A13] whitespace-pre-wrap break-words leading-relaxed">{q.message}</p>
                    <p className="text-[0.55rem] text-[#8C6D58] font-mono mt-1 text-right">
                      {(q.createdAt || "").split("T")[0]}
                    </p>
                  </div>
                </div>
                ...
                {/* Admin bubble (right) */}
                {q.response ? (
                  <div className="flex items-end justify-end gap-2 max-w-full">
                    <div className="max-w-[85%] sm:max-w-[75%] rounded-2xl rounded-br-sm bg-emerald-50 border border-emerald-300 px-3 py-2">
                      <p className="text-[0.6rem] font-bold uppercase tracking-wider text-emerald-800 flex items-center gap-1">
                        <CheckCircle2 size={10} /> {q.respondedBy || "Admin"}
                      </p>
                      <p className="text-xs text-emerald-900 whitespace-pre-wrap break-words leading-relaxed">{q.response}</p>
                      <p className="text-[0.55rem] text-emerald-700/70 font-mono mt-1 text-right">
                        {(q.respondedAt || q.updatedAt || "").split("T")[0]}
                      </p>
                    </div>
                    <div className="shrink-0 h-7 w-7 rounded-full bg-emerald-700 text-white flex items-center justify-center">
                      <ShieldCheck size={13} />
                    </div>
                  </div>
                ) : !isAdmin ? (
                  <div className="flex items-center justify-end">
                    <span className="text-[0.7rem] text-amber-800 bg-amber-50 border border-amber-200 rounded-full px-3 py-1 inline-flex items-center gap-1.5">
                      <Clock size={11} /> Waiting for a response…
                    </span>
                  </div>
                ) : null}

                {/* Admin composer — chat-style */}
                {!q.response && isAdmin && (
                  <div className="pt-1 mt-1 border-t border-[#D4AF37]/20">
                    <label className="sr-only" htmlFor={`reply-${q.id}`}>Reply</label>
                    <div className="flex items-end gap-2">
                      <textarea
                        id={`reply-${q.id}`} rows={1}
                        value={replyDraft[q.id] || ""}
                        onChange={(e) => setReplyDraft((prev) => ({ ...prev, [q.id]: e.target.value }))}
                        onKeyDown={(e) => {
                          if (e.key === "Enter" && !e.shiftKey && (replyDraft[q.id] || "").trim()) {
                            e.preventDefault();
                            handleReply(q);
                          }
                        }}
                        placeholder="Type a reply — Enter to send, Shift+Enter for a new line"
                        className={`${fieldCls} rounded-full px-4 py-2.5 resize-none min-h-11`}
                        style={{ fontSize: "16px" }}
                      />
                      <button
                        ...
                        className="shrink-0 h-11 w-11 rounded-full bg-[#4A0A13] hover:bg-[#5C0F1A] text-[#F5D77F] flex items-center justify-center cursor-pointer disabled:opacity-50"
                      >
                        {replyingId === q.id ? <Loader2 size={14} className="animate-spin" /> : <Send size={14} />}
                      </button>
                    </div>
                  </div>
                )}
