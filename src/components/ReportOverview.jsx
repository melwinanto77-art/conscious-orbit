...
export default function ReportOverview({ report, moduleResults = {}, onClose }) {
  ...
  // Paywall: published reports are DEMO until the client pays.
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs">
      ...
      >
        ...
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <p className="text-xs text-[#7A1C29] font-mono">
              ...
              {isDemo && <span className="ml-2 inline-flex items-center gap-1 rounded-full bg-amber-100 text-amber-800 border border-amber-300 px-2 py-0.5 text-[0.6rem] font-bold uppercase"><Lock size={9} /> Demo</span>}
              {!pending && payment.free && !paid && <span className="ml-2 inline-flex items-center gap-1 rounded-full bg-emerald-100 text-emerald-800 border border-emerald-300 px-2 py-0.5 text-[0.6rem] font-bold uppercase">✓ Free access · granted by admin</span>}
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        {/* Paywall banner — unpaid published reports show a demo */}
        {isDemo && (
          <>
          <div className="rounded-2xl border-2 border-amber-300 bg-gradient-to-br from-amber-50 to-[#FAF4E8] p-5 space-y-3">
            <div className="flex items-start gap-3">
              <div className="h-10 w-10 rounded-xl bg-[#400A12] text-[#F5D77F] flex items-center justify-center shrink-0">
                <Lock size={18} />
              </div>
              <div className="flex-1">
                <p className="text-sm font-bold text-amber-900">This is a demo of your report</p>
                <p className="text-xs text-amber-800 mt-0.5">
                  Score, verdict, module findings, and the Word download unlock after payment.
                  An invoice is issued to admin as soon as payment clears.
                </p>
              </div>
              <div className="text-right shrink-0">
                <p className="font-serif text-2xl font-extrabold text-[#B8860B] leading-none">{currencySymbol}{payment.price}</p>
          ...
          <div className="relative rounded-2xl border border-[#D4AF37]/60 bg-white p-5 space-y-4 overflow-hidden">
            <div className="flex items-center gap-2">
              <span className="text-[0.6rem] uppercase font-bold text-[#B8860B] tracking-wider">Sample preview</span>
              <span className="text-[0.65rem] text-[#8C6D58]">Example of what the full report looks like</span>
            </div>

            <div className="select-none pointer-events-none blur-[3px] space-y-4">
              <div className="rounded-2xl border-2 border-[#D4AF37] bg-white p-5 flex flex-wrap items-center gap-x-8 gap-y-3">
                <div className="text-center">
                  ...
                  <p className="font-serif text-4xl font-extrabold text-[#B8860B]">87<span className="text-lg text-[#8C6D58]"> / 100</span></p>
                </div>
                <span className="px-3 py-1 rounded-full border text-xs font-extrabold bg-emerald-100 text-emerald-800 border-emerald-300">
                  GO / PROCEED
                </span>
                <p className="text-xs italic text-[#7A1C29] flex-1 min-w-52">
                  "Strong market pull with defensible unit economics; pricing headroom validated across two ICP segments."
                </p>
              </div>
              ...
              <div className="rounded-2xl border border-[#D4AF37]/40 bg-white p-5 space-y-3">
                ...
                <p className="text-xs text-[#4A0A13]">
                  Sample rationale: The combination of a repeatable acquisition motion and a 62% gross margin
                  makes this a Go with two conditions — protect pricing power against the incumbent and formalise
                  a channel partnership before Q3.
                </p>
                <div className="grid grid-cols-3 gap-4 text-xs">
                  <div>
                    <p className="font-bold text-emerald-700">Strengths</p>
                    <p className="text-[#4A0A13]">• Repeat purchase rate 46%</p>
                    <p className="text-[#4A0A13]">• LTV/CAC 3.4×</p>
                  </div>
                  <div>
                    <p className="font-bold text-red-700">Risks</p>
                    <p className="text-[#4A0A13]">• Channel concentration</p>
                    <p className="text-[#4A0A13]">• 2 known substitutes</p>
                  </div>
                  <div>
                    <p className="font-bold text-[#B8860B]">Next actions</p>
                    <p className="text-[#4A0A13]">• Sign 1 anchor client</p>
                    <p className="text-[#4A0A13]">• File trademark</p>
                  </div>
                </div>
              </div>

              <div className="rounded-2xl border border-[#D4AF37]/40 bg-white p-5 space-y-2">
                <p className="font-mono text-[0.65rem] uppercase font-bold text-[#B8860B] tracking-wider">Module scores (sample)</p>
                {["Customer Discovery", "Market Size", "Pricing", "GTM", "OKR"].map((m, i) => {
                  const s = [78, 82, 74, 88, 71][i];
                  return (
                    <div key={m} className="flex items-center justify-between gap-3">
                      <span className="text-xs font-medium text-[#4A0A13]">{m}</span>
                      <div className="flex items-center gap-2 w-40">
                        <div className="flex-1 bg-[#4A0A13]/10 rounded-full h-1.5 overflow-hidden">
                          <div className="bg-[#B8860B] h-1.5 rounded-full" style={{ width: `${s}%` }} />
                        </div>
                        <span className="font-mono text-xs font-bold text-[#400A12] w-6 text-right">{s}</span>
                      </div>
                    </div>
                  );
                })}
            ...
            <div className="absolute inset-0 flex items-center justify-center bg-gradient-to-t from-[#FAF4E8]/95 via-transparent to-transparent pointer-events-none">
              <div className="pointer-events-auto flex flex-col items-center gap-2 pb-2">
                <span className="inline-flex items-center gap-1 rounded-full bg-[#400A12] text-[#F5D77F] px-3 py-1 text-[0.65rem] font-bold uppercase">
                  <Lock size={11} /> Example content
                </span>
                <button
                  onClick={() => setShowPay(true)}
                  className="rounded-full bg-[#B8860B] hover:bg-[#D4AF37] text-[#400A12] text-xs font-bold px-4 py-1.5 cursor-pointer"
                >
        ...
        {!pending && !isDemo && report.adminScore !== null && report.adminScore !== undefined && (
        ...
        <div className="rounded-2xl border border-[#D4AF37]/60 bg-[#F5EAD4]/70 p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          ...
          <div className="flex flex-col sm:flex-row gap-2 shrink-0">
            {/* The server-rendered .docx is the document the admin approved,
                including their written analysis — so it is the one to offer
                first. The client-side .doc stays as an offline fallback. */}
            {report.id && (
              <button
                onClick={() =>
        ...
        {!pending && !isDemo && scoredModules.length > 0 && (
